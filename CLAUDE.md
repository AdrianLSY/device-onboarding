...
- **Sleep freeze (`macos/setup/tweak/claude-sleep-freeze.sh` + brew `sleepwatcher`):** Claude Code CLI sessions each hold ~30 TLS connections that overflow the Wi-Fi chip's sleep offload budget and drain ~4%/hr lid-closed. The tweak writes `~/.sleep`/`~/.wakeup` hooks that SIGSTOP every process named exactly `claude` at system sleep and SIGCONT on wake (never the `Claude` app shell), keeping Find My intact (`tcpkeepalive` untouched); measured ≤0.2%/hr overnight. Actions log to `~/.local/state/sleepwatcher.log`. Rollback: `brew services stop sleepwatcher && rm ~/.sleep ~/.wakeup`. ⚠ Dry-run hook changes from a neutral launchd context (`launchctl submit -l probe -- /bin/sh <script>`): Claude-spawned shells cannot pgrep/pkill `claude` processes and report false zero-matches.
